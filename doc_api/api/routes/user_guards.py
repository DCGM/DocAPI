...
# this decorator must be used on route handlers that use challenge_user_access_to_new_job for the documentation to be correct
def uses_challenge_user_access_to_new_job(fn):
...
async def challenge_user_access_to_new_job(
    ...
    if code == AppCode.JOB_NOT_FOUND:
        raise DocAPIClientErrorException(
            ...
            detail=USER_ACCESS_TO_NEW_JOB_GUARD_RESPONSES[AppCode.JOB_NOT_FOUND]["detail"]
        )
    ...
    if db_job.owner_key_id != key.id:
        raise DocAPIClientErrorException(
            ...
            detail=USER_ACCESS_TO_NEW_JOB_GUARD_RESPONSES[AppCode.API_KEY_FORBIDDEN_FOR_JOB]["detail"]
        )
...
WORKER_ACCESS_TO_PROCESSING_JOB_GUARD_RESPONSES = {
    ...
    AppCode.JOB_NOT_IN_PROCESSING: {
        "status": fastapi.status.HTTP_409_CONFLICT,
        "description": "The job is not in the PROCESSING state.",
        "model": DocAPIResponseClientError,
        "detail": "Only jobs in PROCESSING state can be accessed by workers for this operation.",
    }
}
